...
do
    ...
    [  $?  ] && [  x$PDF != 'x'  ]  && [ ! -f  $TD/`basename $ff  .bib`_PDF.jpg  ]  && echo "PDF downloading:  $PDF"   &&  curl -L -o     $WorkD/$F1  $PDF

    [ ! -f  $TD/`basename $ff  .bib`_arXiv.jpg    ]   &&    [ x$PDFURL != 'x' ]  && echo "arXiv downloading:  $PDFURL" && curl -L -o     $WorkD/$F2  $PDFURL
