...
//
// modified by Chunhua Shen, Aug 2008
//
...
function BibTexEntry(text) 
{
   ...
   for (var k in pairs) 
   {
      ...
      if (pair.length>2)
      { // to get rid of whitespace fields, fix !
         ...
         // if no key found, then it contains "," and should be 
         // appended to the previous key
         if ( keyvalue.length < 2 )
         {
            this[key] += ", " + bibtex.trim(pair); 
            continue;
         }
         ...
         for (var ln = 2; ln < keyvalue.length; ln++)
            val += "=" + bibtex.trim(keyvalue[ln]);
...
BibTexEntry.prototype.render = function(database)
{
   ...
   switch (this.type.toLowerCase()) {
      ...
      if (this.volume) 
         string += ", " + span("volume",this.volume);
      ...
      if (this.pages) 
         string += ", pages " +   span("pages",this.pages); 


      if (this.year) 
         string += ", " +  span("month",this.month) + " " + span("year",this.year);


      if (this.publisher)
         string += ". " +  span("publisher",this.publisher);
      ...
      if (this.volume) 
         string += ", volume " + span("volume",this.volume);
      ...
      if (this.address) 
         string += ". " +  span("address",this.address);
   ...
   //
   // add links, DOI, PDF, PS.GZ etc 
   //
   if (this.pdf)
   {
      string += "<br />";
      string += "<a href='" + this.pdf + "' class='pdf'>";
      string += "[PDF]</a>";
   }
   ...
   if (this.arxiv)
   {
      ...
      string += "<a href='http://arxiv.org/abs/" + this.arxiv + "' class='arxiv'>";
      string += "[arXiv]</a>";
   ...
   if (this.project)
   {
      ...
      string += "<a href='./" + this.project + "' class='project'>";
      string += "[Project page]</a>";
...
//    {
//       string += "<br />";
//       string += "<a href='" + this.ps + "' class='ps'>";
//       string += "[PS.GZ]</a>";
//    }
   ...
   for (var i in strings) {
      ...
      if (text) {
         try {
            ...
            this.nrEntries++;
            if (entry.type == "person" || entry.type == "Person")
...
Database.prototype.keysToList = function(keys) 
{
   ...
   for (var i in keys) {
      ...
      {
         idx++;
         rev = keys.length - idx + 1;
         if ( idx % 2 )
            string += "<li class='even' value = " +  rev + " >" + this.render(keys[i]) + "</li>";
         else
...
Database.prototype.replaceSearches = function() 
{
   ...
   for(var i=0; i<tags.length; i++)
   {
      ...
      try
      {
         ...
         if (keys.length <= 0) throw  "null";
 
         if (debug) 
            element.textContent = this.keysToList(keys);
         else 
         {
...
//
// added by Chunhua Shen, Sep 2008
// to return the number of hits
//
Database.prototype.countHits = function() 
{
   var tags = document.getElementsByName("hits");
   ...
   for(var i=0; i<tags.length; i++)
   {
      ...
      try
      {
         ...
         var keys = this.search3(str);
         setTagHTML(element, keys.length);
      }
      catch (err) 
      {
         setTagHTML(element,bibtex.error("search for \"" + str + "\": " + err));
